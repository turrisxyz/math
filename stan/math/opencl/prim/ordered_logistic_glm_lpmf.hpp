...
return_type_t<T_x, T_beta, T_cuts> ordered_logistic_glm_lpmf(
  ...
  check_size_match(function, "Columns of ", "x", N_attributes, "Size of",
                   "beta", stan::math::size(beta));

  const auto& cuts_val = eval(value_of(cuts));
  if (N_classes >= 2) {
    auto cuts_head = block_zero_based(cuts_val, 0, 0, stan::math::size(cuts) - 1, 1);
    auto cuts_tail = block_zero_based(cuts_val, 1, 0, stan::math::size(cuts) - 1, 1);
